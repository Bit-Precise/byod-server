...
function sendNativeMessage(name, args) {
    const message = { channel: 'byod-browser', name, args };
...
function sourceOrigin(config) {
    ...
    if (!value)
        throw new Error('exam source origin is missing');
...
function sourcePage(config) {
    ...
    if (!value)
        throw new Error('exam source page is missing');
    const result = new URL(value);
    if (result.protocol !== 'https:' || !result.hostname || result.username ||
...
function endpointAddress(config) {
    ...
    if (!value)
        throw new Error('exam tunnel endpoint is missing');
...
async function tunnelProof(ticket, endpointID, nonce) {
    const key = await crypto.subtle.importKey('raw', bytes(ticket), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']);
    ...
    return new Uint8Array(await crypto.subtle.sign('HMAC', key, prefix));
}
function allowedOrigins(config) {
    const document = config.policy.document || {};
    const result = new Set(document.allowed_origins || []);
    for (const origin of document.navigation?.allowed_origins || [])
        result.add(origin);
...
async function activateTunnel(config, sessionID, token) {
    ...
    if (!response.ok)
        throw new Error(`exam tunnel ticket rejected (${response.status})`);
    const ticket = await response.json();
    if (!ticket.ticket || !ticket.endpoint_id)
        throw new Error('exam tunnel ticket is incomplete');
    ...
    setNativeFullscreen(browser?.require_fullscreen === true, browser?.lock_fullscreen === true);
}
function clearTimers() {
    ...
    if (startPollTimer !== undefined)
        window.clearInterval(startPollTimer);
...
async function bootstrap(target) {
    ...
    if (!configResponse.ok)
        throw new Error('exam configuration fetch failed');
    ...
    if (!existingSession || !existingToken) {
        ...
        if (!response.ok)
            throw new Error('exam session creation failed');
...
function examLabel(exam) {
    if (exam.completed || exam.state === 'ended')
        return 'Exam ended';
    if (exam.starts_at && Date.parse(exam.starts_at) > Date.now())
...
async function loadAvailableExams() {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error('exam list failed');
