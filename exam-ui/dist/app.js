// byod_server/exam-ui/app.ts
var serviceOrigin = window.location.origin;
var storageToken = "byod.browser_token";
var storageSession = "byod.session_id";
var storageExam = "byod.exam_id";
var title = document.querySelector("#title");
var message = document.querySelector("#message");
var entrySection = document.querySelector("#entry-section");
...
var embeddedInBrowserShell = window.parent !== window;
var nativeBridgeReady = !embeddedInBrowserShell;
var nativeBridgeQueue = [];
window.addEventListener("message", (event) => {
  if (!embeddedInBrowserShell || event.source !== window.parent || event.origin !== "grips://exam.cs.ac.cn")
    return;
  const message2 = event.data;
  if (message2?.channel !== "byod-browser" || message2.type !== "ready") return;
...
function sendNativeMessage(name, args) {
  const message2 = { channel: "byod-browser", name, args };
  if (!embeddedInBrowserShell) return false;
  if (!nativeBridgeReady) {
    nativeBridgeQueue.push(message2);
    return true;
  }
  window.parent.postMessage(message2, "grips://exam.cs.ac.cn");
  return true;
}
var SessionUnauthorizedError = class extends Error {
  constructor() {
...
function setNativeFullscreen(required, locked) {
...
async function tunnelProof(ticket, endpointID, nonce) {
...
function allowedOrigins(config) {
  const document2 = config.policy.document || {};
  const result = new Set(document2.allowed_origins || []);
  for (const origin of document2.navigation?.allowed_origins || []) result.add(origin);
  return [...result].filter((origin) => /^https?:\/\/[^/?#]+$/.test(origin));
}
async function activateTunnel(config, sessionID, token) {
  const path = config.tunnel?.ticket_path?.replace("{session_id}", encodeURIComponent(sessionID)) || `/v1/sessions/${encodeURIComponent(sessionID)}/tunnel-ticket`;
  ...
  setNativeFullscreen(
    browser?.require_fullscreen === true,
    browser?.lock_fullscreen === true
  );
}
function clearTimers() {
  if (heartbeatTimer !== void 0) window.clearInterval(heartbeatTimer);
  if (countdownTimer !== void 0) window.clearInterval(countdownTimer);
  if (startPollTimer !== void 0) window.clearInterval(startPollTimer);
  heartbeatTimer = void 0;
  countdownTimer = void 0;
  startPollTimer = void 0;
}
...
async function completeCurrent(reason = "manual") {
  ...
  showEnded(reason === "manual" ? "Exam submitted. This student cannot enter it again." : "The exam end time was reached. This student cannot enter it again.");
  return true;
}
async function reportViolation(examOrigin, sessionId, token, type) {
...
function showWaiting(target, config, sessionID) {
  ...
  scheduleElement.textContent = `Starts: ${formatTime(config.exam.starts_at)} \xB7 Ends: ${formatTime(config.exam.ends_at)}`;
  entrySection.hidden = true;
  errorElement.hidden = true;
  launch.hidden = true;
  end.hidden = true;
  ...
  countdownTimer = window.setInterval(() => updateCountdown(config.exam.starts_at, "Starts in"), 1e3);
...
function showReady(target, config, sessionID) {
  ...
  const heartbeatMs = Math.max(5e3, Math.min(3e5, configuredHeartbeat * 1e3));
  ...
  if (config.exam.ends_at) {
    countdownTimer = window.setInterval(() => {
      ...
      if (new Date(config.exam.ends_at).getTime() <= Date.now()) {
...
async function bootstrap(target) {
  ...
  if (config.exam?.id !== id || config.exam.origin !== target.origin || !config.policy?.signature || !config.policy.key_id || config.policy.document?.exam_id !== id) {
    throw new Error("invalid exam configuration");
  }
  currentConfig = config;
  const pageURL = new URL(window.location.href);
  const existingSession = pageURL.searchParams.get("session_id") || (localStorage.getItem(storageExam) === id ? localStorage.getItem(storageSession) : null);
...
function beginConnectLogin() {
  const destination = embeddedInBrowserShell ? "grips://exam.cs.ac.cn/?auth=1" : `${serviceOrigin}/?auth=1`;
...
function showExamChoices(exams) {
  clearTimers();
  clearNativeExamState();
  ...
  for (const exam of exams) {
    ...
    detail.textContent = `${examLabel(exam)} \xB7 starts ${formatTime(exam.starts_at)} \xB7 ends ${formatTime(exam.ends_at)}`;
...
async function selectExam(exam) {
  ...
  try {
    const response = await fetch(new URL("/v1/sessions", serviceOrigin), {
      ...
      credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ exam_id: exam.id, return_uri: embeddedInBrowserShell ? `grips://exam.cs.ac.cn/?target=${encodeURIComponent(target.href)}` : url.toString() })
    });
    if (response.status === 401) {
      showEntry();
      entryError.textContent = "Your Connect login has expired. Sign in again.";
...
var directHTTPSExamShell = !embeddedInBrowserShell && window.location.protocol === "https:" && window.location.hostname === "exam.cs.ac.cn" && !window.location.port;
